...
async function fetchAndDisplayAgents(page) {
    ...
    // --- تحسين: تحديد الحقول المطلوبة فقط لزيادة الأداء والأمان ---
    const requiredColumns = `
        id, name, agent_id, classification, rank, 
        avatar_url, telegram_channel_url, telegram_group_url, 
        created_at
    `;
    let query = supabase.from('agents').select(requiredColumns, { count: 'exact' });
    ...
    if (searchTerm) {
        query = query.or(`name.ilike.%${searchTerm}%,agent_id.ilike.%${searchTerm}%`);
    }
    ...
    query = query.range(startIndex, startIndex + AGENTS_PER_PAGE - 1);
    ...
    displayAgentsPage(agents, page, count);
}
